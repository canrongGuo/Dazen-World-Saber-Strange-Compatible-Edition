...
Game_BattlerBase.prototype.canEquipWeapon = function(item) {
    var isValid = this.isEquipWtypeOk(item.wtypeId);
    if (!isValid && item.meta && item.meta.lx) {
        isValid = item.meta.lx.match(/(\d?)[^,]/g).some(function(type) {
            return this.isEquipWtypeOk(parseInt(type));
        }.bind(this));
    }
    return isValid && !this.isEquipTypeSealed(item.etypeId);
};
...
Game_Actor.prototype.isWtypeEquipped = function(wtypeId) {
    return this.weapons().some(function(weapon) {
        ...
        if (weapon.meta && weapon.meta.lx) {
            return weapon.meta.lx.match(/(\d?)[^,]/g).some(function(type) {
                return parseInt(type) === wtypeId;
            });
        }
    });
};
