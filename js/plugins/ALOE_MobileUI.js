...
* @param toumingdu
* @text 透明度
* @desc 变更虚拟按键透明度1-Parameters['toumingdu']
* @default 100
* @help
* 
* //=============================================================================
...
(function () {
    ...
    Game_System.prototype.initialize = function() {
        Alias._Game_System_initialize.apply(this, arguments);
        this._mobileUIhideAll = false;
    };
	...
	Sprite_Button.prototype.initialize = function (x, y, normalImage, soundEffect, vibratePattern) {
		...
		this.active = true;
		this.z = 5;
		this.opacity = Parameters['toumingdu'];
	};
	...
	Sprite_Button.prototype.updateVisibility = function () {
		if (this._hiding && this.opacity > 0) {
			this.opacity -= Parameters['toumingdu'] / this._duration;
		} else if (!this._hiding && this.opacity < Parameters['toumingdu']) {
			this.opacity += Parameters['toumingdu'] / this._duration;
		}
	};

	Sprite_Button.prototype.updateActive = function () {
		if (this.opacity === Parameters['toumingdu']) {
			this.active = true;
		}
	};
	...
	Sprite_Button.prototype.hide = function () {
		...
		this.active = false;
        if ($gameSystem) {
            $gameSystem._mobileUIhideAll = $gameSystem._mobileUIhideAll || true;
        }
	};
	...
	Sprite_Button.prototype.show = function () {
		this._hiding = false;
        if ($gameSystem) {
            $gameSystem._mobileUIhideAll = $gameSystem._mobileUIhideAll || false;
            $gameSystem._mobileUIhideAll = false;
            this.visible = true;
        }
	};
	...
	Sprite_DirectionalPad.prototype.initialize = function (x, y, image, soundEffect) {
		...
		this._lastInput = '';
		this._hiding = false;
        if ($gameSystem && $gameSystem._mobileUIhideAll) {
            this._hiding = true;
            this.active = false;
            this.visible = false;
        }
	...
	Sprite_ControlButton.prototype.hideAllButtons = function () {
		...
		Object.keys(this._keyButtons).forEach(function (a) {
            if ($gameSystem) {
                _this2._mobileUIhideAll = $gameSystem._mobileUIhideAll;
            }
			_this2._keyButtons[a].hide();
            if ($gameSystem && $gameSystem._mobileUIhideAll) {
                $gameSystem._mobileUIhideAll = _this2._mobileUIhideAll;
            }
            return;
		});
		Object.keys(this._keyButtons).forEach(function (a) {
		...
		if (this._directionalPad) {
            if ($gameSystem) {
                this._mobileUIhideAll = $gameSystem._mobileUIhideAll;
            }
			this._directionalPad.hide();
            if ($gameSystem && $gameSystem._mobileUIhideAll) {
                $gameSystem._mobileUIhideAll = this._mobileUIhideAll;
            }
		}
		if (this._directionalPad) {
	...
	Scene_Base.prototype.createDirPad = function () {
		...
		if (params) {
			if (params.activeScenes.length > 0 && params.activeScenes.contains(this.constructor)) {
                ...
                if ($gameSystem && $gameSystem._mobileUIhideAll) {
                    this._directionalPad._hiding = true;
                    this._directionalPad.active = false;
                    this._directionalPad.visible = false;
                }
			...
			if (params.length > 0) {
				...
				for (var i = 0; i < params.length; i++) {
					if (params[i].activeScenes.length > 0 && params[i].activeScenes.contains(this.constructor)) {
						...
						if ($gameSystem && $gameSystem._mobileUIhideAll) {
                            this._keyButtons[a.name.toLowerCase()]._hiding = true;
                            this._keyButtons[a.name.toLowerCase()].active = false;
                            this._keyButtons[a.name.toLowerCase()].visible = false;
                        }
	...
	Scene_Base.prototype.createControlButton = function () {
		...
		if (params) {
			if (params.activeScenes.length > 0 && params.activeScenes.contains(this.constructor)) {
                ...
                if ($gameSystem && $gameSystem._mobileUIhideAll) {
                    this._controlButton._hiding = true;
                    this._controlButton.active = false;
                    this._controlButton.visible = false;
                }
	...
	Scene_Base.prototype.showMobileUI = function () {
		...
		if (this._directionalPad) {
            if ($gameSystem) {
                !$gameSystem._mobileUIhideAll && this._directionalPad.show();
            } else {
                this._directionalPad.show();
            }
		}
		if (this._keyButtons) {
			Object.keys(this._keyButtons).forEach(function (a) {
                if ($gameSystem) {
                    return !$gameSystem._mobileUIhideAll && _this4._keyButtons[a].show();
                } else {
		...
		if (this._controlButton) {
            if ($gameSystem) {
                !$gameSystem._mobileUIhideAll && this._controlButton.show();
            } else {
	...
	Scene_Map.prototype.processMapTouch = function () {
		...
		if (Parameters['disableTouchMovement'] && this._directionalPad && this._directionalPad.active && $gameSystem && !$gameSystem._mobileUIhideAll) {
			return;
		}
		if (TouchInput.isTriggered()) {
			var point = new Point(TouchInput.x, TouchInput.y);
			if (!!this._controlButton) {
				if (this._controlButton.containsPoint(point) && $gameSystem && !$gameSystem._mobileUIhideAll) {
					return;
				}
			}
			if (this._directionalPad) {
				if (this._directionalPad.active && this._directionalPad.containsPoint(point) && $gameSystem && !$gameSystem._mobileUIhideAll) {
					return;
				}
			}
			if (this._keyButtons && $gameSystem && !$gameSystem._mobileUIhideAll) {
	...
	Window_Selectable.prototype.processTouch = function () {
		if (Parameters['disableTouchWindows'].contains(SceneManager._scene.constructor.name) && $gameSystem && !$gameSystem._mobileUIhideAll) {
			return;
		}
