...
Window_ShopCommand.prototype.createCommand = function(command) {
    ...
    if (command === 'BUY') {
      ...
      this.addCustomCommands();
        ...
        this.addCommand(unescape('%u4E00%u952E%u51FA%u552E'), 'Onekeysell', !this._purchaseOnly);
    }
};
...
Window_ShopStatus.prototype.drawDarkRectEntries = function() {
    for (var i = 0; i < 8; ++i) {
	  // 位置调整
      var rect = this.getRectPosition(i*2);
      this.drawDarkRect(rect.x + 220, rect.y, rect.width, rect.height);
    }
};
...
Window_ShopStatus.prototype.drawActorStatInfo = function(actor) {
    ...
    for (var i = 0; i < 8; ++i) {
	  ...
	  // 修改显示位置
      this.drawText(text, rect.x + 220, rect.y, rect.width);
      if (!canEquip) this.drawActorCantEquip(actor, rect, 220);
      if (canEquip) this.drawActorChange(actor, rect, item1, i, 220);
    }
    this.changePaintOpacity(true);
};

Window_ShopStatus.prototype.drawActorCantEquip = function(actor, rect, x) {
    this.changePaintOpacity(false);
    this.resetTextColor();
    this.contents.fontSize = Yanfly.Param.ShopCantSize;
    var text = '-';
    this.drawText(text, rect.x + x, rect.y, rect.width, 'right');
};

Window_ShopStatus.prototype.drawActorChange = function(actor, rect, item1, i, x) {
...
Window_ShopStatus.prototype.drawActorEquipInfo = function(x, y, actor) {
    ...
    this.resetFontSettings();
	// 位置调整
    this.drawText(actor.name(), x + 220, y, this.contents.width - x);
    var item1 = this.currentEquippedItem(actor, this._item.etypeId);
    if (enabled) {
      this.contents.fontSize = Yanfly.Param.ShopStatFontSize;
