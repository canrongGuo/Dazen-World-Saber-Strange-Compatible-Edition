...
ItemManager.removeAugmentFromSlot = function(item, slotId) {
    ...
    if (list && list.length > 0)  {
      this.processAugmentList(item, augment, slotId, list, false);
    }
...
ItemManager.installAugmentToSlot = function(item, effectItem, slotId) {
    ...
    if (list && list.length > 0) {
      this.processAugmentList(item, effectItem, slotId, list, true);
    }
...
ItemManager.processAugmentList = function(item, effectItem, slotId, list, zt) {
  var length = list.length;
  for (var i = 0; i < length; ++i) {
    var line = list[i];
    this.processAugmentEffect(line, item, effectItem, slotId, zt);
  }
};

ItemManager.processAugmentEffect = function(line, mainItem, effectItem, slot, zt) {
	//元素
...
Window_ItemInfo.prototype.drawAugmentInfo = function(dy) {
  ...
  if (Yanfly.Param.AugmentInfoTitle !== '') {
	...
	// 居中对齐
    this.drawText(Yanfly.Param.AugmentInfoTitle, this.textPadding() - 38, dy,
...
Window_ItemInfo.prototype.drawAugmentData = function(slot, dy) {
    ...
    // 居中对齐
	this.drawTextEx(text, this.textPadding() + 220, dy);
};
