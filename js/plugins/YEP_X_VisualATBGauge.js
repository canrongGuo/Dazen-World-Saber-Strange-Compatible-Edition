...
Yanfly.VATB.Sprite_Actor_preSpriteInitialize =
    Sprite_Actor.prototype.preSpriteInitialize;
Sprite_Actor.prototype.preSpriteInitialize = function(battler) {
    Yanfly.VATB.Sprite_Actor_preSpriteInitialize.call(this, battler);
    this.createVisualATBWindow();
};

Yanfly.VATB.Sprite_Actor_update = Sprite_Actor.prototype.update;
Sprite_Actor.prototype.update = function() {
    Yanfly.VATB.Sprite_Actor_update.call(this);
    this.addVisualATBWindow();
};
...
Sprite_Actor.prototype.createVisualATBWindow = function() {
    if (!BattleManager.isATB()) return;
    this._visualATBWindow = new Window_ActorVisualATB();
};

Yanfly.VATB.Sprite_Actor_setBattler = Sprite_Actor.prototype.setBattler;
Sprite_Actor.prototype.setBattler = function(battler) {
    Yanfly.VATB.Sprite_Actor_setBattler.call(this, battler);
    if (this._visualATBWindow) this._visualATBWindow.setBattler(battler);
};

//=============================================================================
// Window_ActorVisualATB
//=============================================================================

function Window_ActorVisualATB() {
    this.initialize.apply(this, arguments);
}

Window_ActorVisualATB.prototype = Object.create(Window_Base.prototype);
Window_ActorVisualATB.prototype.constructor = Window_ActorVisualATB;

Window_ActorVisualATB.prototype.initialize = function() {
    Window_Base.prototype.initialize.call(this, 0, 0, 1, 1);
    this._battler = null;
    this._requestRefresh = false;
    this._currentATBSpeed = 0;
...
Window_ActorVisualATB.prototype.setBattler = function(battler) {
    if (this._battler === battler) return;
    this._battler = battler;
};

Window_ActorVisualATB.prototype.update = function() {
    Window_Base.prototype.update.call(this);
    if (!this._battler) return;
    this.updateWindowAspects();
};

Window_ActorVisualATB.prototype.updateWindowAspects = function() {
    this.updateWindowSize();
    this.updateWindowPosition();
    this.updateOpacity();
    this.updateATBPosition();
...
Window_ActorVisualATB.prototype.updateWindowSize = function() {
    var spriteWidth = this._battler.atbGaugeWidth();
    var width = spriteWidth + this.standardPadding() * 2;
    width = Math.min(width, Graphics.boxWidth + this.standardPadding() * 2);
    var height = this.lineHeight() + this.standardPadding() * 2;
...
Window_ActorVisualATB.prototype.makeWindowBoundaries = function() {
    if (!this._requestRefresh) return;
    this._minX = -1 * this.standardPadding();
    this._maxX = Graphics.boxWidth - this.width + this.standardPadding();
    this._minY = -1 * this.standardPadding();
...
Window_ActorVisualATB.prototype.updateWindowPosition = function() {
    if (!this._battler) return;
    var battler = this._battler;
    this.x = battler.spritePosX();
    this.x -= Math.ceil(this.width / 2);
...
Window_ActorVisualATB.prototype.updateOpacity = function() {
    if (this.isShowWindow()) {
      this.contentsOpacity += 32;
    } else {
      this.contentsOpacity -= 32;
...
Window_ActorVisualATB.prototype.maxOpacity = function() {
    return this._battler.battler().opacity;
};

Window_ActorVisualATB.prototype.isShowWindow = function() {
    if (!this._battler.isAppeared()) return false;
    if (!this._battler.showATBGauge()) return false;
    if (this._battler.isDead()) return false;
    if (Yanfly.Param.VATBAlwaysShow) return true;
...
Window_ActorVisualATB.prototype.updateATBPosition = function() {
    if (!this._battler) return;
    if (this._currentATBSpeed !== this._battler.atbSpeed()) {
      this._currentATBSpeed = this._battler.atbSpeed()
      this._requestRefresh = true;
...
Window_ActorVisualATB.prototype.updateRefresh = function() {
    if (this._requestRefresh) this.refresh();
};

Window_ActorVisualATB.prototype.refresh = function() {
    this.contents.clear();
    if (!this._battler) return;
    this._requestRefresh = false;
    var wy = this.contents.height - this.lineHeight();
...
Window_ActorVisualATB.prototype.drawGauge =
function(dx, dy, dw, rate, color1, color2) {
  var color3 = this.gaugeBackColor();
  var fillW = Math.floor(dw * rate).clamp(0, dw);
  var gaugeH = this.gaugeHeight();
