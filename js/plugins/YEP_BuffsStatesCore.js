 ...
 * Introduction
 * ============================================================================
 * 2020年4月23日21:20:58 修了个小bug
  *如果你用了ATB并且回合结束状态效果不生效试试将这段放在代码里
...
Yanfly.BSC.Game_Action_itemEffectAddNormalState = Game_Action.prototype.itemEffectAddNormalState;
Game_Action.prototype.itemEffectAddNormalState = function(target, effect) {
	if(target.isStateAffected(effect.dataId)){
	var stateId = effect.dataId;
	var state = $dataStates[stateId];
    target._stateTurns[stateId] = target._stateTurns[stateId] || 0;
    var variance = 1 + Math.max(state.maxTurns - state.minTurns, 0);		
	target._stateTurns[stateId] += Math.randomInt(variance);  			
	}
	Yanfly.BSC.Game_Action_itemEffectAddNormalState.call(this, target, effect);
};
...
Game_BattlerBase.prototype.resetStateCounts = function(stateId) {
    ...
    if (state.reapplyRules === 1) {
      ...
      Yanfly.BSC.Game_BattlerBase_resetStateCounts.call(this, stateId);
		...
		this._stateTurns[stateId] += Math.randomInt(variance);  
	  }
...
Game_Battler.prototype.typestates = function(type) {
    ...
    if(this.need_refreshstatelist[type] || this._statelist[type].length == 0){
...
Game_Battler.prototype.onTurnStartStateEffects = function() {
    var states = this.typestates('turnStartState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
...
Game_Battler.prototype.onTurnEndStateEffects = function() {
    var states = this.typestates('turnEndState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
...
Game_Battler.prototype.onRegenerateStateEffects = function() {
    var states = this.typestates('regenerateState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
...
Game_Battler.prototype.onActionStartStateEffects = function() {
    var states = this.typestates('actionStartState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
...
Game_Battler.prototype.onActionEndStateEffects = function() {
    var states = this.typestates('actionEndState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
...
Game_Action.prototype.onApplyStateEffects = function(target) {
    var states = this.subject().typestates('initiateState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
      if (!state) continue;
...
Game_Action.prototype.onSelectStateEffects = function(target) {
    var states = target.typestates('selectState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
      if (!state) continue;
...
Game_Action.prototype.onDeselectStateEffects = function(target) {
    var states = target.typestates('deselectState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
      if (!state) continue;
...
Game_Action.prototype.offApplyStateEffects = function(target) {
    var states = this.subject().typestates('concludeState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
      if (!state) continue;
...
Game_Action.prototype.onPreDamageStateEffects = function(target, value) {
    var states = this.subject().typestates('confirmState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
      if (!state) continue;
...
Game_Action.prototype.onReactStateEffects = function(target, value) {
    var states = target.typestates('reactState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
      if (!state) continue;
...
Game_Action.prototype.onRespondStateEffects = function(target, value) {
    var states = target.typestates('respondState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
      if (!state) continue;
...
Game_Action.prototype.onPostDamageStateEffects = function(target, value) {
    var states = this.subject().typestates('establishState');
    var length = states.length;
    for (var i = 0; i < length; ++i) {
      var state = states[i];
      if (!state) continue;
