...
//lv+用于判断是否处于范围(选中目标,需要判断的组群【玩家组/敌人组等，常用:target.opponentsUnit().aliveMembers()】,技能范围【像素】,技能范围高度比率【不常用】)
var addAoeCircleTargets_yanLong = function(target, targets, radiusValue, heightRate) {
    var main = target;
    if (!main) return;
    var group = targets;
    ...
    for (var i = 0; i < length; ++i) {
      var member = group[i];
      if (isInsideAoeCircle_yanLong(main, member, radiusValue, heightRate)) {
...
//lv++++++++++++++++++++
var isInsideAoeCircle_yanLong = function (main, target, radiusValue, heightRate) {
