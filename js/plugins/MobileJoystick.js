 ...
 * @on 启用
 * @off 禁用
 * @desc 非手机端是否启用按钮，true - 启用，false - 禁用
 * @default true
 * 
 * @param 按钮图片
 * @desc 选择按钮使用的图片素材
...
MobileJoystick.menuBtnPCActive = [];
...
for (var i = 0; i < 20; i++) {
    ...
    MobileJoystick.mapBtnPCActive[i] = String(MobileJoystick.mapBtn[i]['PCWeb有效']).toLowerCase() == 'true';
...
for (var i = 0; i < 15; i++) {
    ...
    MobileJoystick.btlBtnPCActive[i] = String(MobileJoystick.btlBtn[i]['PCWeb有效']).toLowerCase() == 'true';
...
for (var i = 0; i < 5; i++) {
    ...
    MobileJoystick.menuBtnPCActive[i] = String(MobileJoystick.menuBtn[i]['PCWeb有效']).toLowerCase() == 'true';
...
Scene_Base.prototype.createMapButton = function() {
    ...
    for (var i = 0; i < MobileJoystick.mapBtn.length; i++) {
        if (MobileJoystick.mapBtnActive[i] && (Utils.isMobileDevice() || MobileJoystick.mapBtnPCActive[i])) {
...
Scene_Base.prototype.createBattleButton = function() {
    ...
    for (var i = 0; i < MobileJoystick.btlBtn.length; i++) {
        if (MobileJoystick.btlBtnActive[i] && (Utils.isMobileDevice() || MobileJoystick.btlBtnPCActive[i])) {
...
Scene_Base.prototype.createMenuButton = function() {
    ...
    for (var i = 0; i < MobileJoystick.menuBtn.length; i++) {
        if (MobileJoystick.menuBtnActive[i] && (Utils.isMobileDevice() || MobileJoystick.menuBtnPCActive[i])) {
...
Scene_Map.prototype.update = function() {
    ...
    if (!MobileJoystick.Message && (TouchInput.isPressed() || MobileJoystick.Touch || MobileJoystick.DirTouch)) {
        ...
        for (var i = 0; i < 20; i++) {
            if (MobileJoystick.mapBtnActive[i] && (Utils.isMobileDevice() || MobileJoystick.mapBtnPCActive[i])) {
                var xx = x - MobileJoystick.mapBtnX[i] - this._mapBtn[i].width / 2;
                var yy = y - MobileJoystick.mapBtnY[i] - this._mapBtn[i].height / 2;
                if ((MobileJoystick.mapBtnMode[i] || TouchInput.isTriggered()) && Math.sqrt(Math.pow(xx, 2) + Math.pow(yy, 2)) <= this._mapBtn[i].width / 2)
                    this.updateButton(i);
            }
        }
...
Scene_Battle.prototype.update = function() {
    ...
    if (!MobileJoystick.Message && (TouchInput.isPressed() || MobileJoystick.Touch || MobileJoystick.DirTouch)) {
        ...
        for (var i = 0; i < 15; i++) {
            if (MobileJoystick.btlBtnActive[i] && (Utils.isMobileDevice() || MobileJoystick.btlBtnPCActive[i])) {
                var xx = x - MobileJoystick.btlBtnX[i] - this._btlBtn[i].width / 2;
                var yy = y - MobileJoystick.btlBtnY[i] - this._btlBtn[i].height / 2;
                if ((MobileJoystick.btlBtnMode[i] || TouchInput.isTriggered()) && Math.sqrt(Math.pow(xx, 2) + Math.pow(yy, 2)) <= this._btlBtn[i].width / 2)
                    this.updateButton(i);
            }
        }
        if (MobileJoystick.Shadow && this._touchShadow) this.updateShadow();
...
Scene_MenuBase.prototype.update = function() {
    ...
    if (TouchInput.isPressed() || MobileJoystick.Touch || MobileJoystick.DirTouch) {
        ...
        for (var i = 0; i < 5; i++) {
            if (MobileJoystick.menuBtnActive[i] && (Utils.isMobileDevice() || MobileJoystick.menuBtnPCActive[i])) {
                var xx = x - MobileJoystick.menuBtnX[i] - this._menuBtn[i].width / 2;
                var yy = y - MobileJoystick.menuBtnY[i] - this._menuBtn[i].height / 2;
                if ((MobileJoystick.menuBtnMode[i] || TouchInput.isTriggered()) && Math.sqrt(Math.pow(xx, 2) + Math.pow(yy, 2)) <= this._menuBtn[i].width / 2)
                    this.updateButton(i);
            }
        }
        if (MobileJoystick.Shadow && this._touchShadow) this.updateShadow();
...
Window_Selectable.prototype.processTouch = function () {
    if (MobileJoystick.Scenes.contains(SceneManager._scene.constructor.name) && (Utils.isMobileDevice() || MobileJoystick.PCJoystick))
        return;
