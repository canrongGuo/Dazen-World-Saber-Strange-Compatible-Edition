 ...
 * @type boolean
 *
 * @help 是可以在游戏中表现时间和天气的概念的插件。
 * 自动，地图移动，战斗时间经过，与时间一起天气和色调变化。
 * 这些时间可以调节，在活动中会停止时间进行。
 *
 * 而且，也有在游戏中反映现实时间的功能。
 * 设定内容和现实的时间在游戏中的链接。
 *
 * 日期和星期日也会记录，可以自由地设定星期的数量和名称。
 * 现在日付按照格式，在画面左上显示。
 *
 * 在日期格式中可以使用以下。
 ...
 *
 * 另外，如果准备了按照规格的图像，也可以显示模拟表。
 * 显示位置可以调整各图像的显示是否。
 *
 * 图像的规格如下。
 *  文字盘：任意大小的正方形图像
 *  长针：用与文字盘相同尺寸的图像，指上（0）的针的图像
 *  短针：用与文字盘相同尺寸的图像，指上（0）的针的图像
 *
 ...
 *  （在引数之间用半角空间划分）
 *
 * C_ADD_TIME [分] : 指定的增加时间的数值（分钟）。
 * C_ADD_DAY [日] : 指定的增加时间的数值（日）。
 * C_SET_TIME [時] [分] : 更改指定时间。例子：C_SET_TIME 11 20
 * C_SET_DAY [年] [月] [日] : 更改指定时间。
 * C_STOP : 时间停止
 * C_START : 时间恢复
 * C_SHOW : 显示日历。
 * C_HIDE : 不显示日历。
 * C_DISABLE_TINT : 禁止根据时间段改变色调。
 * C_ENABLE_TINT : 允许变更时间段的色调。
 * C_DISABLE_WEATHER : 禁止时间经过的天气变化。
 * C_ENABLE_WEATHER : 允许时间经过的天气变化。
 * C_SET_SNOW_LAND : 坏天气的时候会下雪。
 * C_RESET_SNOW_LAND : 坏天气的时候会下雨或是暴风雨。
 * C_SET_SPEED [分] : 实际时间设定为1秒的时间的经过速度。
 * C_SHOW_CLOCK : 显示模拟表。
 * C_HIDE_CLOCK : 不显示模拟表。
 * C_SET_TIME_REAL : 将时间的取得方法变更为实际时间。
 * C_SET_TIME_VIRTUAL : 将时间的取得方法变更为游戏内时间。
 * C_SET_RAINY_PERCENT [確率] : 设置降水概率（0-100）。
 * C_INIT_TOTAL_TIME : 累计时间，累计天数初始化。
 *
 * ・模拟时钟图像变更指令
 * 可以改变模拟表的图像文件名(img/pictures)。
 * 但是，实际的图像被变更的是移动地图之后。
 * C_SET_CLOCK_BASE [ファイル名] : 更改文字盘图像的文件名。
 * C_SET_HOUR_HAND [ファイル名] : 短針画像のファイル名を変更します。
 * C_SET_MINUTE_HAND [ファイル名] : 長針画像のファイル名を変更します。
 *
 * ・计时器操作命令
 * 从命令执行指定的时间[分]后，开关和自助开关
 * ON可以命令。
 * 也可以与实际时间联动功能一起使用。
 * 在开关的情况下，ID，在自助开关的情况下，指定种类（A，B，C，D）。
 *
 * C_SET_SWITCH_TIMER [分] [スイッチID] [ループ]
 * 指定示例（在游戏内时间经过30分钟后，开启[10] 为ON）
 * C_SET_SWITCH_TIMER 30 10 ON
 *
 * C_SET_SELF_SWITCH_TIMER [分] [セルフスイッチ種類] [ループ]
 * 规格示例（在游戏时间3小时后打开自动开关[B]（*））
 * C_SET_SELF_SWITCH_TIMER 180 B OFF
 * ※目标事件是执行插件命令的事件。
 *
 * 如果中间有可能取消或暂停，请执行命令指定[计时器名称]。 这是因为在取消时需要指定计时器名称。
 *
 * C_SET_SWITCH_NAMED_TIMER [タイマー名] [分] [スイッチID] [ループ]
 * 指定例（在游戏内时间经过30分钟后，开关[ 10 ]为ON）
 * C_SET_SWITCH_NAMED_TIMER timer 30 10 ON
 *
 * C_SET_SELF_SWITCH_NAMED_TIMER [タイマー名] [分] [セルフスイッチ種類] [ループ]
 * 指定例（在游戏内时间过3小时后将自我开关[ B ]（显示）为ON）
 * C_SET_SELF_SWITCH_NAMED_TIMER timer 180 B OFF
 * ※目标事件是执行插件命令的事件。
 *
 * 解除、停止、重新开始的命令如下。
 * C_CLEAR_TIMER timer # 取消定时器名“timer”。
 * C_STOP_TIMER timer  # 暂时停止计时器名“timer”。
 * C_START_TIMER timer # 重启定时器名“timer”。
 *
 * 备注栏详细
 *  如果在紧贴和图上的笔记栏中输入以下的话，会自动将天气和色调变化自动作废。
 *  在室内地图和活动场景等想暂时无效化的情况下可以使用。
 *  设置将优先在地图上的备注里。
 *
 * <C_Tint:OFF>    # 将色调的变更暂时无效化。
 * <C_色調:OFF>    # 同上
 * <C_Weather:OFF> # 将天气暂时无效化。
 * <C_天候:OFF>    # 同上
 *
 ...
 *
 * 高度的设定
 * 参照源代码中的“用户交换区域”，可以改变以下内容。
 *  时间段的信息（早上从几点到几点）
 *  每小时的色调（但是，如果天气恶劣的情况下会有修正）
 *
 * 使用规约：
 * 可擅自改换、重新分发，利用形式（商务、18禁利用等）
 * 关于也没有限制。
 * 这个插件已经是你的。
 */
