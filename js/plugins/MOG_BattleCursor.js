...
/*:
 * @plugindesc (v2.2) 战斗 - 单位选择指针
 * @author Moghunter （Drill_up翻译+优化）
 *
 * @param 资源-友军指针
 * @desc 指向友军的指针图片资源。
 * @default 战斗单位选择指针-友军指针
 * @require 1
 * @dir img/system/
 * @type file
 *
...
BattleCursor.prototype.createArrowActor = function() {
  ...
  for (var i = 0; i < $gameParty.maxBattleMembers(); i++) {
	   this._actor_arrow[i] = new Sprite(ImageManager.loadSystem(Moghunter.src_BattleCursor_A));
...
BattleCursor.prototype.createArrowEnemy = function() {
   for (var i = 0; i < this.enemies().length; i++) {
	   this._enemy_arrow[i] = new Sprite(ImageManager.loadSystem(Moghunter.src_BattleCursor_B));
