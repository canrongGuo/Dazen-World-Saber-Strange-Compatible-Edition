...
(function() {
	...
	var SightFontSize = +(parameters['fontSize'] || 20);
	...
	var SightTextAlign = parameters['textAlign'] || 'left';
	...
	Sprite_FutureSight.prototype.initialize = function(enemySprite) {
		...
		//var width = SightWidth;//这里修改黑框长度
		var width = 170;
		var height = 222; // 目眩 行数
	...
	Sprite_FutureSight.prototype.refresh = function() {
		...
		if (this._texts.length > 0) {
			...
			var height = 24 * lines; // 目眩 单行高度
			this.drawSightBack(0, y, width, height);
			this.drawSightHeader(0, 0, width, 10);
			this.drawSightText(4, y, width - 8, 24);// 目眩 单行高度
		}
	};

	Sprite_FutureSight.prototype.drawSightBack = function(x, y, width, height) {
		//this.bitmap.paintOpacity = SightBackOpacity;//这里修改透明度
		this.bitmap.paintOpacity = 150;
	...
	Sprite_FutureSight.prototype.drawSightText = function(x, y, width, height) {
		this.bitmap.fontSize = 18; // 目眩 文本大小
		...
		for (var i = 0; i < this._texts.length; i++) {
			...
			if (iconIndex > 0) {
				...
				this.drawSightIcon(iconIndex, x + iconX, y-9);//这里调节图标坐标
			}
			var arr = text.split('\n');
	...
	Sprite_FutureSight.prototype.update = function() {
		...
		this.y = this._enemySprite.y + (+enemy.meta.fsOffsetY || 0)+150;
	};
