...
Window_CommonEventMenu.prototype.makeCommandList = function() {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      if (this.includes(ce)) {
        ...
        //var name = ce.menuSettings.name;//这里去掉选项图标
...
Window_CommonEventMenu.prototype.drawItem = function(index) {
    ...
    this.resetTextColor();
    this.drawTextEx(this.commandName(index), rect.x, rect.y);//这里修改选项中文字位置
	//var abc='\\c[3]' + this.commandName(index) + '\\c[0]';
    //this.drawTextEx(abc, rect.x, rect.y);
};
...
Scene_Base.prototype.commonEventMenuSetupList = function(id) {
   ...
   $gameVariables.setValue(99,id);//这里保存当前执行事件的DATAID
