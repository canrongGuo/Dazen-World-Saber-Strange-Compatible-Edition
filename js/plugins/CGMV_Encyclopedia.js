...
CGMV_Encyclopedia.prototype.initializeData = function(array, length, symbol) {
	...
	for(var i = 0; i < length; i++) {
		if(gameData[i+1] && gameData[i+1].meta && !gameData[i+1].meta.cgmvencyclopediahide) continue;//这里修改进图鉴的东西 加了个感叹号代表有标签才进
...
CGMV_Scene_Encyclopedia.prototype.createListWindow = function() {
	var width = Graphics.boxWidth/5;//这里改左边列表窗口大小
...
CGMV_Scene_Encyclopedia.prototype.createDummyWindow = function() {
	...
	var y = this._titleWindow.height + this._categoryWindow.height;
	var width = Graphics.boxWidth*4/5;//这里修改右边窗口大小，互补
...
CGMV_Window_EncyclopediaDisplay.prototype.drawItem = function() {
	...
	var y = this.drawUserTPGain(item.tpGain, this.lineHeight()*3);
	//y = this.drawEncyclopediaEffects(item.effects, y);
	y = this.drawEncyclopediaDescription(item.description, y+this.lineHeight()*2);
...
CGMV_Window_EncyclopediaDisplay.prototype.drawArmor = function() {
	...
	//this.drawEncyclopediaPossession($gameParty.numItems(armor));这里
	...
	var y = this.drawEncyclopediaTrait(armor.traits, this.lineHeight()*5);
...
CGMV_Window_EncyclopediaDisplay.prototype.drawSkill = function() {
	...
	var y = this.drawUserTPGain(skill.tpGain, this.lineHeight()*2);
	//y = this.drawEncyclopediaEffects(skill.effects, y);
	y = this.drawEncyclopediaDescription(skill.description, y+this.lineHeight()*3);
