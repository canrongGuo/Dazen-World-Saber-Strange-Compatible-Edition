...
Window_StatCompare.prototype.drawItem = function(x, y, paramId) {
    ...
    this.drawParamName(y, paramId, 220);
    this.drawCurrentParam(y, paramId, 60);
    this.drawRightArrow(y, 60);
    if (!this._tempActor) return;
    this.drawNewParam(y, paramId , 45);
    this.drawParamDifference(y, paramId, 40);
};
...
Window_StatCompare.prototype.drawParamName = function(y, paramId, w) {
    var x = this.textPadding();
    this.changeTextColor(this.systemColor());
    this.drawText(TextManager.param(paramId).substring(0,4), x + w, y, this._paramNameWidth);
};

Window_StatCompare.prototype.drawCurrentParam = function(y, paramId, w) {
    ...
    this.drawText(actorparam, x + w, y, this._paramValueWidth, 'right');
};

Window_StatCompare.prototype.drawRightArrow = function(y, w) {
    var x = this.contents.width - this.textPadding();
    x -= this._paramValueWidth + this._arrowWidth + this._bonusValueWidth;
    var dw = this.textWidth('\u2192' + ' ');
    this.changeTextColor(this.systemColor());
    this.drawText('\u2192', x + w, y, dw, 'center');
};

Window_StatCompare.prototype.drawNewParam = function(y, paramId, w) {
    var x = this.contents.width - this.textPadding();
...
Window_StatCompare.prototype.drawParamDifference = function(y, paramId) {
    ...
    this.drawText(text, x + 40, y, this._bonusValueWidth, 'left');
};
