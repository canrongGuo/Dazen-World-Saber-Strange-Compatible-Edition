 ...
 * Introduction
 * ============================================================================
 * 布乌的自用半汉化版本，只翻译了我觉得需要翻译的部分。
 * 警告：这个插件只能用于MV1.5.0以上版本。 
 *
 * ---
 *
 ...
 *
 * 你可以调整任务标题, 展示难度等级, 以及告诉玩家们这个任务的委托人是谁,Balabala
 * where that quest is from, various dynamic
 ...
 *
 * Category Order
 * - 这个参数将决定以下分类按钮在菜单中出现的顺序：
 * available（进行中）, completed（已完成）, failed（失败）, all（所有）, cancel（取消）. 
 * Moving these around will let you
 ...
 *
 * Available Text
 * - 在这里填入该分类的文本，可以使用文本代码来改变文本，如图标、字体大小、颜色等。
 * %1到时候会自动替换为该分类下的任务数量。
 ...
 *
 * 窗口设置
 * - 如果你想自定义分类窗口的话, 你可以通过设置下面这些参数来调整窗口的属性。
 * 不过如果你不是很懂JS语言的话，还是不要改这些内容比较好。乱改的话，后果自负。
 * Default: 
 *
 * X: 0
 * Y: 0
 ...
 *
 * ============================================================================
 * Instructions - 设置任务列表窗口（按任务性质分类）
 * ============================================================================
 *
 * 'Quest List Window'修改这个插件属性，可以显示不同种类的任务. 默认有以下四种：
 * Main Quests（主线）, Side Quests（支线）, Character Quests（角色任务）, 以及
 * Tutorial Quests（教程任务？）. 
 * 下面是可改参数。
 * ---
 *
 * Show Types（显示种类，默认显示）
 ...
 *           "\\c[5]Tutorial Quests"]
 *
 * List Open Symbol（展开列表，这个参数没必要修改）
 ...
 * Default: 0
 *
 * Show Empty（决定某分类下没任务时是否显示该分类，默认是不显示）
 ...
 *
 * ============================================================================
 * Instructions - 设置任务数据窗口（这个比较重要）
 * ============================================================================
 *
 * 这个参数 'Quest Data Window' 用于展示任务的具体内容. 
 * This data is used by the player to
 ...
 * over the world.
 *
 * Word Wrap Version（自动换行）(需要前置插件 YEP_MessageCore.js):
 ...
 * over the world.
 *
 * Quest Data Format（任务内容格式，自动替换的东西来源于具体的任务设置）
 * - 用于向玩家更生动地展示你的任务内容. 
 * %1 自动替换为标题，没法加图标和颜色。
 * %2 自动替换为任务难度等级. 
 * %3 自动替换为任务委托人. 
 * %4 自动替换为委托人所在地. 
 * %5 显示当前任务内容，该内容在中途可以被改变.
 * %6 任务目标. 
 * %7 任务奖励.
 * %8 注释（可有可无）
 *
 * Default:（这些文字都是可以改的，文本图标和颜色都可以自己设置）
 * \{%1\}
 * \c[4]Level:\c[0] %2
 * \c[4]From:\c[0] %3
 * \c[4]Location:\c[0] %4
 ...
 * %8
 *
 * Word Wrap Version (Requires YEP_MessageCore.js):（如果有信息核心插件的话就用这个版本）
 * <WordWrap>\{%1\}
 * <br>\c[4]Level:\c[0] %2
 * <br>\c[4]From:\c[0] %3
 ...
 *
 * Description（任务描述）
 * - 用于添加详细的任务描述. 你可以增加很多页任务描述. 但是默认只会显示第一页的内容. 
 * 如果有多页描述，可以使用插件指令:'Quest x Change Description Entry To y'
 * 在任务进行中途改变描述内容。
 *
 * Objectives List（目标步骤列表）
 ...
 *
 * ============================================================================
 * Main Menu Manager - 在主菜单中加入任务按钮（好像不用自己加，插件装上菜单中就有任务按钮了）
 * ============================================================================
 *
 ...
 *
 * 记得要关掉插件中的 'Auto Place Command' 按钮。
 *
 * ============================================================================
 * Script Calls脚本代码
 * ============================================================================
 *
 * 这个插件允许你在事件中用不同的脚本代码来监控任务的进程。 
 * 下面列出了适用的事件类型及可用脚本代码。
 *
 ...
 * (ie. 'Main Quests', 'Side Quests', 'Character Quests', etc). 替换时要加引号。
 * Example: $gameSystem.totalQuestTypes('all', 'Main Quests')
 *
 * $gameSystem.getQuestDescriptionIndex(questId)
 * - Replace 'questId' 替换为指定任务的ID. 用于显示当前使用的任务描述。This will
 ...
 * - Replace 'questId' with the ID of the quest you're looking for.用于显示该任务的可见目标数量 
 * This will set the variable to show how many quest objectives are visible currently for
 * the selected quest.
 * Example: $gameSystem.totalVisibleQuestObjectives(50)
 *
 * $gameSystem.totalQuestObjectives(questId)
 * - Replace 'questId' with the ID of the quest you're looking for. 显示所有目标数量。This will
 ...
 * - Replace 'questId' with the ID of the quest you're looking for. 可见奖励This will
 ...
 * - Replace 'questId' with the ID of the quest you're looking for. 所有奖励This will
 ...
 * - Replace 'questId' with the ID of the quest you're looking for. 注释检索This will
 ...
 *           条件分歧中可使用的脚本代码
 * 
 * $gameSystem.isQuestObjectiveCompleted(questId, objectiveId)检查某任务的某目标是否已完成。
 ...
 *
 * $gameSystem.isQuestObjectiveFailed(questId, objectiveId)检查某任务的某目标是否是已失败。
 * - Replace 'questId' with the ID of the quest you're looking for. Replace
 * 'objectiveId' with the ID of the objective you're intending to check. This
 * will make a check in the conditional branch's script call to see if an
 ...
 *
 * $gameSystem.isQuestObjectiveUncleared(questId, objectiveId)检查某任务的某目标是否未完成。
 * - Replace 'questId' with the ID of the quest you're looking for. Replace
 * 'objectiveId' with the ID of the objective you're intending to check. This
 * will make a check in the conditional branch's script call to see if an
 ...
 * 
 * $gameSystem.isQuestRewardClaimed(questId, rewardId)检查某任务的某奖励是否被领取。
 * - Replace 'questId' with the ID of the quest you're looking for. Replace
 ...
 * 
 * $gameSystem.isQuestRewardDenied(questId, rewardId)检查某任务的某奖励是否被拒绝？。
 * - Replace 'questId' with the ID of the quest you're looking for. Replace
 * 'rewardId' with the ID of the reward you're intending to check. This will
 * make a check in the conditional branch's script call to see if a reward's
 ...
 * 
 * $gameSystem.isQuestRewardUnclaimed(questId, rewardId)检查某任务的某奖励是否未领取。
 * - Replace 'questId' with the ID of the quest you're looking for. Replace
 * 'rewardId' with the ID of the reward you're intending to check. This will
 * make a check in the conditional branch's script call to see if a reward's
 ...
 *   若该指令选中的是一个玩家未知的任务，这个任务将会被添加到任务系统中。
 *   ---
 *
 *   Quest Journal Show
 *   Quest Journal Hide
 ...
 *
 *   Quest Add x
 *   - Replace 'x' with an 整数. Adds quest ID 'x' to the quest journal as
 *   an available quest. This will make it viewable from the in-game quest
 *   journal system menu.
 *
 *   Quest Add x to y
 *   - Replace 'x' and 'y' with 整数 values determining the quest ID range
 *   you wish to add in mass amounts to the quest journal.
 *
 *   Quest Add x, x, x
 *   - Replace 'x' values with 整数 values representing the quest ID's you
