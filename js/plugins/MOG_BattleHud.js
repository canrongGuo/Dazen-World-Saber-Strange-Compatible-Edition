...
Battle_Hud.prototype.need_refresh_parameter = function(parameter) {
  switch (parameter) {
  	...
  	case 1:
         ...
         break;			
  	case 2:
         if (this._tp_old[0] != this._battler.barrierPoints()) {return true};
		 if (this._tp_old[1] != this._battler.def) {return true};
         break;					
  };
  return false;
};
...
Battle_Hud.prototype.create_tp_number = function() {
	...
	this._tp_number_old = this._battler.barrierPoints();
...
Battle_Hud.prototype.create_maxtp_number = function() {
	...
	this._maxtp_number_old = this._battler.def;
...
Battle_Hud.prototype.update_tp = function() {
	if (this._tp_meter_blue) {
		if(this._tp_flow[0]) {
		   this.refresh_meter_flow(this._tp_meter_blue,this._battler.barrierPoints(),this._battler.def,0,this._tp_flow[1]);
	   	   var dif_meter = this.update_dif(this._tp_old_ani[0],this._battler.barrierPoints(),160)
		   if (this._tp_old_ani[0] != dif_meter) {this._tp_old_ani[0] = dif_meter;
	       this.refresh_meter_flow(this._tp_meter_red,this._tp_old_ani[0],this._battler.def,1,this._tp_flow[1]);
		   };
		   this._tp_flow[1] += 1.5;
		   if (this._tp_flow[1] > this._tp_flow[3]) {this._tp_flow[1] = 0};		   
			...
			if (this.need_refresh_parameter(2)) {
				this.refresh_meter(this._tp_meter_blue,this._battler.barrierPoints(),this._battler.def,0);
				this._tp_old = [this._battler.barrierPoints(),this._battler.def];
			};
			var dif_meter = this.update_dif(this._tp_old_ani[0],this._battler.barrierPoints(),160)
			if (this._tp_old_ani[0] != dif_meter) {this._tp_old_ani[0] = dif_meter;
			this.refresh_meter(this._tp_meter_red,this._tp_old_ani[0],this._battler.def,1);};
	};
    };
	if (this._tp_number) {
		var dif_number = this.update_dif(this._tp_number_old,this._battler.barrierPoints(),30)
...
Battle_Hud.prototype.refresh_states2 = function() {
	...
	if(this._battler._equips[0]._itemId>0){//这里修改战斗者装备栏第一个位置的装备图标
	var icons=[$dataWeapons[this._battler._equips[0]._itemId].iconIndex];
	}else{
	var icons=[0];
	}
	//console.log(icons);
