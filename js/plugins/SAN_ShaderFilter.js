 ...
 * @help
 * - Overview
 * This plugin provides visual effect functions with shader filter.
 ...
 * In addition to this plugin, sub plugins are necessary
 * to display the visual effect.
 * 
 * - External file of GLSL shader code
 * This plugin uses external files in which GLSL code is described.
 * The GLSL shader code is placed in the "glsl" folder.
 * Do not to delete or change "glsl" folder.
 * 
 * - Basic mechanism
 * Each sprites watch each game objects.
 ...
 * and reflects it as a visual effect on the game screen display.
 * In other words, you can display a visual effect
 * by registering a filter parameter object in the game object.
 * 
 * For the sake of explanation, change it as follows.
 ...
 *        It is an operation such as fadein/fadeout.
 *        Operate by calling FPObject processing.
 *        Specifically, refer to the help of each sub plugin.
 * 
 * - Operation on FPMap
 ...
 * register each value to be used for the filter.
 * 
 * Example:
 * $gameScreen.glslFilterParamMap().set(
 *     'filterId', // Filter ID
 ...
 * Delete the FPObject by specifying the filterID
 * from the FPMap of the game object.
 * 
 * Example:
 * $gameScreen.glslFilterParamMap().delete(
 ...
 * Check whether the FPObject of the filter ID specified
...
// 如果SAN_ShaderFlag不为true/false则判断为开启
if(localStorage.SAN_ShaderFlag != 'false' && localStorage.SAN_ShaderFlag != 'true'){
	localStorage.SAN_ShaderFlag = true;
};
var Star_ConfigManagerData_SAN_ShaderFilter = ConfigManager.applyData;
ConfigManager.applyData = function(config) {
    Star_ConfigManagerData_SAN_ShaderFilter.call(this, config);
    this.SAN_ShaderFlag = config.SAN_ShaderFlag;
};
var Star_ConfigManagermakeData_SAN_ShaderFilter = ConfigManager.makeData;
ConfigManager.makeData = function() {
    var config = Star_ConfigManagermakeData_SAN_ShaderFilter.call(this);
        config.SAN_ShaderFlag = !(localStorage.SAN_ShaderFlag == 'false');
    return config;
};
var Star_makeCommandList_SAN_ShaderFilter = Window_Options.prototype.makeCommandList;
Window_Options.prototype.makeCommandList = function() {
    Star_makeCommandList_SAN_ShaderFilter.call(this);
    this.addCommand('SAN_ShaderFlag', 'SAN_ShaderFlag');
};
ConfigManager.SAN_ShaderFlag = !(localStorage.SAN_ShaderFlag == 'false');
var Star_Window_OptionsprocessOk =Window_Options.prototype.processOk;
Window_Options.prototype.processOk = function() {
    Star_Window_OptionsprocessOk.call(this);
	if(this.commandSymbol(this.index()) == 'SAN_ShaderFlag'){
		localStorage.SAN_ShaderFlag = ConfigManager.SAN_ShaderFlag;
	}
};
