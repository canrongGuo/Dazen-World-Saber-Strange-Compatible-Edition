...
//
//插件记录：
//		插件附着在 .setValue 方法上。只要setValue，就存储全局变量。
//
...
// ** 全局读取
//=============================================================================
if(Utils.isNwjs() || (DataManager.loadGlobalInfo().toString() !== '') || typeof DataManager.loadGlobalInfo()[0] == 'undefined'){
  ...
  var _drill_global;
  var intervalget_drill_global = setInterval(function() {
   if(DataManager.loadGlobalInfo().toString() !== ''){
	_drill_global = DataManager.loadGlobalInfo();
	//alert(JSON.stringify(_drill_global));
	if( !DrillUp.global_var_switch ){	//游戏没关情况
		if( _drill_global && _drill_global[0] && _drill_global[0]["_global_var_switch"] ){		//游戏关闭后，重开读取global中的配置
			DrillUp.global_var_switch = _drill_global[0]["_global_var_switch"];
	...
	clearInterval(intervalget_drill_global);   //读取成功,清除延时程序  
   }
  }, 500);
}
//=============================================================================
