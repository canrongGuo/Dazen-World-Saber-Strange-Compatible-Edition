...
Yanfly.Param.IBSShowinstatus = eval(String(Yanfly.Parameters['show in status']));
Yanfly.Param.elementshows = eval(String(Yanfly.Parameters['show elements']));
...
Game_System.prototype.initIBSSettings = function() {
  ...
  this._enemyshowInBattleStatus = Yanfly.Param.IBSShowinstatus;
  this._showelements = Yanfly.Param.elementshows;
};
...
Game_System.prototype.setshowelements = function(value) {
  if (this._showelements === undefined) this.initIBSSettings();
  this._showelements = value;
};
//=============================================================================
// Game_Interpreter
//=============================================================================
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
  ...
  if (command === 'ShowInBattleStatus') {
    ...
    $gameSystem.setenemyShowInBattleStatus(false);
  }else if (command === 'ShowEnemyInBattleStatus') {
    $gameSystem.setenemyShowInBattleStatus(true);
  }else if (command === 'HideElements') {
    $gameSystem.setshowelements(false);
  }else if (command === 'ShowElements') {
    $gameSystem.setshowelements(true);
  }
};
...
Window_Base.prototype.drawActorSimpleStatus = function(actor, x, y, width) {
	...
	if(actor.isActor())    this.drawActorClass(actor, x2, y, width2);
...
Window_Base.prototype.drawActorLevel = function(actor, x, y) {
    ...
    this.resetTextColor();
    if(actor.isActor()){var level = Yanfly.Util.toGroup(actor.level);
...
Window_InBattleStatus.prototype.refresh = function() {
  ...
  if (!this._battler.isAppeared) return;
...
/*   if (h >= this.lineHeight() * 8) {
	...
	if($gameSystem._showelements){
    for (var i = 1; i < $dataSystem.elements.length; ++i) {
      this.drawElem(i, x2, y, w/2, this.lineHeight());
      if (i % 4 != 0) {
        x2 += w/2 ;
      } else {
        x2 = x;
...
/* 	this.drawDarkRect(x, y, this.contents.width, this.lineHeight());
	if(Yanfly.Param.IBSShowinstatus){
 	 var b = $gameParty.battleMembers();
	 b = b.concat($gameTroop._enemies);
	}else{
	var b = $gameParty.battleMembers();	
	}

	 var index = b.indexOf(this._battler);
	var xx = x;
	for(var kk = 0;kk<b.length;kk++){
		this.drawActorName(b[kk], xx, y);
		xx += this.textWidth(Yanfly.Util.toGroup(b[kk].name()))+10;
		if(xx >= this.contents.width){
			alert('change')
			xx = x;
			y += this.lineHeight();
		this.drawDarkRect(x, y, this.contents.width, this.lineHeight());
...
Window_InBattleStatus.prototype.drawElem = function(paramId, dx, dy, dw, dh) {
  this.drawDarkRect(dx, dy, dw, dh);
  var level = Math.round(this._battler.elementRate());
  ////var icon = this._battler.buffIconIndex(level, paramId);
  ////this.drawIcon(icon, dx + 2, dy + 2);
  dx += Window_Base._iconWidth + 4;
  dw -= Window_Base._iconWidth + 4 + this.textPadding() + 2;
  this.changeTextColor(this.systemColor());
  this.drawText($dataSystem.elements[paramId], dx-20, dy, dw);
  var value = Math.floor(this._battler.elementRate(paramId)*100);
  this.changeTextColor(this.paramchangeTextColor(-1*value+100));
  this.drawText(Yanfly.Util.toGroup(value)+'%', dx, dy, dw, 'right');
};
...
Window_InBattleStateList.prototype.updateLeftRight = function() {
	if($gameSystem._enemyshowInBattleStatus){
 	  var b = $gameParty.battleMembers();
	  var c = [];
	  for(var j = 0;j<$gameTroop._enemies.length;j++){
		  if($gameTroop._enemies[j].isAppeared()) c.push($gameTroop._enemies[j]);
	  }
	  b = b.concat(c);
	}else{
	var b = $gameParty.battleMembers();	
  ...
  index = index.clamp(0, b.length - 1);
  if (current !== index) {
    var battler = b[index];
...
Scene_Battle.prototype.InBattleStatusOpen = function(actor) {
  this._helpWindow.show();
  this._inBattleStatusWindow.show();
  this._inBattleStateList.show();
  this._inBattleStateList.activate();
