...
Game_BattlerBase.prototype.param = function(paramId) {
  ...
  try {
    var value = eval(code);
    /*var value = new Function("base", "plus", "paramRate", "buffRate", "flat",
      "minValue", "maxValue", "a", "user", "subject",
      "b", "target", "s", "v", "code", "return (" + code + ")")
      .apply(this, [base, plus, paramRate, buffRate, flat, minValue, maxValue, a, user, subject, b, target, s, v, code]);*/
  } catch (e) {
    var value = 0;
...
Game_BattlerBase.prototype.paramMax = function(paramId) {
  ...
  try {
    var value = eval(code);
    /*var value = new Function("customMax", "a", "user", "subject",
      "b", "target", "s", "v", "code", "return (" + code + ")")
      .apply(this, [customMax, a, user, subject, b, target, s, v, code]);*/
  } catch (e) {
    var value = 0;
...
Game_BattlerBase.prototype.paramMin = function(paramId) {
  ...
  try {
    var value = eval(code);
    // var value = new Function("customMin", "a", "user", "subject",
      // "b", "target", "s", "v", "code", "return (" + code + ")")
      // .apply(this, [customMin, a, user, subject, b, target, s, v, code]);
  } catch (e) {
    var value = 0;
...
Game_Battler.prototype.paramRate = function(paramId) {
    var rate = Game_BattlerBase.prototype.paramRate2.call(this, paramId)-1;
    var length = this.states().length;
    for (var i = 0; i < length; ++i) {
      var obj = this.states()[i];
      if (obj && obj.rateParams) rate += (obj.rateParams[paramId]+1); //改状态的地方
    }
    return rate;
};
...
Game_Actor.prototype.paramRate = function(paramId) {
    var rate = Game_Battler.prototype.paramRate2.call(this, paramId)+1;
    rate += (this.actor().rateParams[paramId]-1);
    rate += (this.currentClass().rateParams[paramId]-1);	//改装备角色职业的地方
      var length = this.equips().length;
    for (var i = 0; i < length; ++i) {
      var obj = this.equips()[i];
...
Game_Enemy.prototype.paramRate = function(paramId) {
    var rate = Game_Battler.prototype.paramRate2.call(this, paramId)+1;
    rate += (this.enemy().rateParams[paramId]-1);//敌人的地方
    return rate;
};
